...
import { ArrowLeft, Upload, Download, CheckCircle, XCircle, AlertTriangle, Package, FileSpreadsheet } from 'lucide-react'
...
export default function ProductsImportPage() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      fd.append('file', file)
      const res = await fetch('/api/import/products/preview', { method: 'POST', body: fd })
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-900">Importo Produkte nga Excel</h1>
        </div>
      </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <a href="/api/import/products/template">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-400 mt-2">
              Kolona obligative: <strong>Emri Produktit</strong>, <strong>Kategoria</strong>, <strong>Çmimi</strong>. Shkarko template-in sipër.
            </p>
          </CardContent>
        </Card>
      )}
      ...
      {step === 'preview' && preview && (
        <>
          ...
          {/* Info: products without photo */}
          {preview.rows.some(r => r.valid && !r.photo) && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl p-4">
              <AlertTriangle className="h-4 w-4 text-amber-600 mt-0.5 shrink-0" />
              <p className="text-sm text-amber-700">
                Disa produkte nuk kanë foto URL — do të importohen si <strong>JOAKTIV</strong>.
                Shtoji foton më vonë nga faqja e produktit për t&apos;i aktivizuar.
              </p>
            </div>
          )}
            ...
            <CardContent className="space-y-4">
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-1.5">
                  Nëse kodi i produktit ekziston tashmë:
                </label>
                  ...
                  {(['skip', 'update'] as const).map(opt => (
                    <label key={opt} className={`flex-1 flex items-center gap-2 border rounded-xl px-4 py-3 cursor-pointer transition-colors ${onDuplicate === opt ? 'border-primary bg-primary/5' : 'border-gray-200 hover:bg-gray-50'}`}>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{opt === 'skip' ? 'Produkti ekzistues mbetet i pandryshuar' : 'Mbishkruaj me të dhënat nga Excel'}</p>
                      </div>
                    </label>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <p className="text-sm text-gray-500">
                  Do të importohen <strong className="text-green-700">{preview.stats.valid}</strong> produkte valid
                ...
                <table className="w-full text-sm min-w-[700px]">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Emri Produktit</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Kodi</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Kategoria</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Çmimi</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Stok</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {preview.rows.slice(0, DISPLAY_LIMIT).map(row => (
                      <tr key={row.rowIndex} className={!row.valid ? 'bg-red-50/40' : row.warnings.length > 0 ? 'bg-amber-50/30' : ''}>
                        ...
                        <td className="px-4 py-2.5 font-medium text-gray-800 max-w-[160px] truncate">{row.name || <span className="text-red-400 italic">mungon</span>}</td>
                        <td className="px-4 py-2.5 text-gray-500 font-mono text-xs">{row.code || '—'}</td>
                        <td className="px-4 py-2.5 text-gray-600 max-w-[120px] truncate">{row.category || <span className="text-red-400 italic">mungon</span>}</td>
                        <td className="px-4 py-2.5 text-gray-700 font-medium">
                          {row.salesPrice ? formatCurrency(parseFloat(row.salesPrice)) : <span className="text-red-400 italic">—</span>}
                        </td>
                        <td className="px-4 py-2.5 text-gray-600">{row.initialStock || '0'}</td>
      ...
      {step === 'importing' && (
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-gray-600 font-medium">Duke importuar produktet...</p>
        ...
        <Card>
          <CardContent className="py-8 space-y-6">
            ...
            <div className="flex gap-3 justify-center">
              <Link href="/admin/products">
                <Button>Shko te lista e produkteve</Button>
              </Link>
