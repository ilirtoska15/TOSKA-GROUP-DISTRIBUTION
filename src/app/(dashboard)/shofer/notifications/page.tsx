...
const TYPE_ICON: Record<string, string> = {
  VISIT_PLANNED: '📅', ORDER_PENDING: '📋', OVERDUE_DEBT: '💰',
  LOW_STOCK: '⚠️', PAYMENT_RECEIVED: '💵', ORDER_APPROVED: '✅',
}

export default function AgjentNotificationsPage() {
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold text-gray-900">Njoftimet</h1>
        ...
        {loading ? (
          ...
          <div className="divide-y">
            {notifications.map(n => (
              <div
                key={n.id}
                className={`px-4 py-4 ${!n.isRead ? 'bg-blue-50/40 border-l-2 border-l-primary' : 'hover:bg-gray-50'} transition-colors`}
              >
                <div className="flex items-start gap-3">
                  ...
                  {!n.isRead && (
                    <button onClick={() => markRead(n.id)} className="flex-shrink-0 text-gray-400 hover:text-primary transition-colors" title="Shëno si të lexuar">
